...
@Component({
  selector: 'sngh-jisuushi',
  templateUrl: './jisuushi.component.html'
})
export class JisuushiComponent {
  title = 'jisuushi';
  public input: string;
  ...
  ngOnInit() {
    var textInput = document.getElementById('jisuushiInput');
    wanakana.bind(textInput, { IMEMode: 'ToHiragana' });
    this.http.get('assets/data.txt', {responseType: 'text'})
        .subscribe(data => this.parseJisuushi(data));
  }
